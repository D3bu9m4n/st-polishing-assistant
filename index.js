...
const defaultSettings = {
    ...
    matchingLabel: "<content>,</content>|<theatre>,</theatre>|<Developer_background>,</details>",
...
async function loadSettings() {
    ...
    extension_settings[extensionName].matchingLabel = extension_settings[extensionName].matchingLabel || defaultSettings.matchingLabel;
...
function saveApiSettings() {
    ...
    extension_settings[extensionName].bannedWords = $("#banned_words").val().split(',').map(word => word.trim()).filter(word => word !== '');
...
async function handleIncomingMessage(data) {
    ...
    for (const pair of tagPairs) {
        try {
            ...
            const regex = new RegExp(`${escapeRegex(pair.open)}([\\s\\S]*?)${escapeRegex(pair.close)}`);
            ...
            if (matchResult) {
                ...
                console.log(`[润色助手] Found content using tags: ${pair.open}...${pair.close}`);
                break; // Stop searching once a match is found
            }
        } catch (error) {
            console.error(`[润色助手] Error creating regex for pair ${pair.open}, ${pair.close}:`, error);
    ...
    // 计算纯中文字数（排除标点和空格）
    const chineseCharCount = contentToPolish.replace(/[^\u4e00-\u9fa5]/g, '').length;
    ...
    try {
        ...
        console.log("[润色助手] Sending request to API:", JSON.stringify(requestBody, null, 2)); // Log the request body for debugging
        ...
        if (!result.choices || !result.choices[0] || !result.choices[0].message || typeof result.choices[0].message.content === 'undefined') {
            console.error("[润色助手] API response format unexpected:", JSON.stringify(result));
...
jQuery(async () => {
    try {
        ...
        $("#model_url, #model_name, #api_key, #prompt_text, #banned_words, #matching_label").on("input", saveApiSettings);
        ...
        if (extension_settings[extensionName].enabled) {
            ...
            eventSource.removeListener(event_types.MESSAGE_RECEIVED, handleIncomingMessage); // Ensure no duplicates
            eventSource.on(event_types.MESSAGE_RECEIVED, handleIncomingMessage);
        } else {
